...
# Q Coin Self-Contained Release Builder
...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "Usage: .\create-embedded-release.ps1" -ForegroundColor Yellow
...
Write-Host "🏗️ Building quantum-miner executable..." -ForegroundColor Yellow
...
foreach ($cudaPkg in $cudaPackages) {
    ...
    try {
        ...
        Write-Host "  ⚠️ $cudaPkg failed" -ForegroundColor Yellow
    }
}
...
# Create completely isolated Python wrapper
$pythonWrapper = @'
@echo off
REM Q Coin Isolated Python Wrapper - Does NOT affect system Python
REM Save original environment
set "ORIG_PYTHON_HOME=%PYTHON_HOME%"
set "ORIG_PYTHONPATH=%PYTHONPATH%"
set "ORIG_PATH=%PATH%"

REM Set isolated environment (only for this process)
set "PYTHON_HOME=%~dp0python"
set "PYTHONPATH=%PYTHON_HOME%;%PYTHON_HOME%\Lib;%PYTHON_HOME%\Lib\site-packages"
set "PATH=%PYTHON_HOME%;%ORIG_PATH%"

REM Prevent interference with system Python
set "PYTHONDONTWRITEBYTECODE=1"
set "PYTHONUSERBASE=%PYTHON_HOME%\user"
set "PIP_USER=false"
set "PIP_PREFIX=%PYTHON_HOME%"

REM Execute with isolated environment
"%PYTHON_HOME%\python.exe" %*

REM Restore original environment (cleanup)
set "PYTHON_HOME=%ORIG_PYTHON_HOME%"
set "PYTHONPATH=%ORIG_PYTHONPATH%"
set "PATH=%ORIG_PATH%"
'@
Set-Content -Path (Join-Path $releaseDir "python.bat") -Value $pythonWrapper -Encoding ASCII

# Create completely isolated miner launcher
$minerLauncher = @'
@echo off
...
REM Check if system Python exists (just for info)
python --version >nul 2>&1
if %ERRORLEVEL% eq 0 (
    echo System Python detected - will NOT be affected
) else (
    echo No system Python detected - that's OK!
)

REM Test our isolated Python environment
echo Testing embedded Python environment...
call python.bat -c `"import sys; print(`'Isolated Python:`', sys.version.split()[0], `'at`', sys.executable)`"
if %ERRORLEVEL% neq 0 (
    echo Embedded Python test failed
    pause
    exit /b 1
)

call python.bat -c `"import qiskit; print(`'Qiskit:`', qiskit.__version__)`"
if %ERRORLEVEL% neq 0 (
...
if %ERRORLEVEL% eq 0 (
    ...
    echo GPU not available, using CPU mode (this is normal without NVIDIA GPU)
)
...
echo.
echo Starting quantum miner with isolated Python environment...
echo    Threads: %THREADS%
echo    Node: %NODE%
echo    Coinbase: %COINBASE%
echo    Your system Python remains completely untouched!
echo.

quantum-miner.exe -node %NODE% -coinbase %COINBASE% -threads %THREADS%
'@
Set-Content -Path (Join-Path $releaseDir "start-miner.bat") -Value $minerLauncher -Encoding ASCII

# Create enhanced isolation-focused README
$readme = @'
...
- Wrapper scripts that only affect this miner
...
Built: [TIMESTAMP]
Size: ~150MB (completely self-contained)
'@

# Replace timestamp placeholder
$readme = $readme -replace '\[TIMESTAMP\]', (Get-Date)
Set-Content -Path (Join-Path $releaseDir "README.md") -Value $readme -Encoding UTF8
...
try {
    ...
    Write-Host "⚠️ Test failed but continuing: $_" -ForegroundColor Yellow
}

# Add isolation testing script
Write-Host "🔧 Adding isolation test script..." -ForegroundColor Yellow
$isolationTestPath = Join-Path $ScriptDir "test-isolation.ps1"
if (Test-Path $isolationTestPath) {
    Copy-Item $isolationTestPath (Join-Path $releaseDir "test-isolation.ps1") -Force
    Write-Host "  ✅ Added test-isolation.ps1" -ForegroundColor Green
} else {
    Write-Host "  ⚠️ Isolation test script not found" -ForegroundColor Yellow
}
