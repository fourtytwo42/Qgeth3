...
func TestHeaderInsertion(t *testing.T) {
	...
	hc, err := NewHeaderChain(db, gspec.Config, qmpow.NewFaker(), func() bool { return false })
	if err != nil {
		t.Fatal(err)
	}
	// chain A: G->A1->A2...A128
	genDb, chainA := makeHeaderChainWithGenesis(gspec, 128, qmpow.NewFaker(), 10)
	// chain B: G->A1->B1...B128
	chainB := makeHeaderChain(gspec.Config, chainA[0], 128, qmpow.NewFaker(), genDb, 10)
