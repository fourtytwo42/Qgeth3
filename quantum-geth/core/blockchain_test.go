...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, comparator func(td1, td2 *big.Int), scheme string) {
	...
	genDb, _, blockchain2, err := newCanonical(qmpow.NewFaker(), i, full, scheme)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.chainConfig, blockchain2.GetBlockByHash(blockchain2.CurrentBlock().Hash()), n, qmpow.NewFaker(), genDb, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.chainConfig, blockchain2.CurrentHeader(), n, qmpow.NewFaker(), genDb, forkSeed)
...
func testLastBlock(t *testing.T, scheme string) {
	genDb, _, blockchain, err := newCanonical(qmpow.NewFaker(), 0, true, scheme)
	if err != nil {
	...
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 1, qmpow.NewFullFaker(), genDb, 0)
...
func testExtendCanonical(t *testing.T, full bool, scheme string) {
	...
	_, _, processor, err := newCanonical(qmpow.NewFaker(), length, full, scheme)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(qmpow.NewFaker(), 10, full, scheme)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 5, qmpow.NewFaker(), genDb, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 5, qmpow.NewFaker(), genDb, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(qmpow.NewFaker(), 0, full, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), qmpow.NewFaker(), genDb, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), qmpow.NewFaker(), genDb, len(second), func(i int, b *BlockGen) {
...
func testBadHashes(t *testing.T, full bool, scheme string) {
	...
	if full {
		blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 3, qmpow.NewFaker(), genDb, 10)
		...
		headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 3, qmpow.NewFaker(), genDb, 10)
...
func testReorgBadHashes(t *testing.T, full bool, scheme string) {
	// Create a pristine chain and database
	genDb, gspec, blockchain, err := newCanonical(qmpow.NewFaker(), 0, full, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	// Create a chain, import and ban afterwards
	headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 4, qmpow.NewFaker(), genDb, 10)
	blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 4, qmpow.NewFaker(), genDb, 10)
	...
	ncm, err := NewBlockChain(blockchain.db, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool, scheme string) {
	doTest := func(i int) {
		...
		if full {
			blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), i, qmpow.NewFaker(), genDb, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.engine = qmpow.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), i, qmpow.NewFaker(), genDb, 0)
...
func testFastVsFullChains(t *testing.T, scheme string) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	defer ancient.Stop()
...
func testLightVsFastVsFullChainHeads(t *testing.T, scheme string) {
	...
	height := uint64(1024)
	_, blocks, receipts := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), int(height), nil)
	...
	archive, _ := NewBlockChain(archiveDb, &archiveCaching, gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	lightDb := makeDb()
	defer lightDb.Close()
	light, _ := NewBlockChain(lightDb, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
...
func testChainTxReorgs(t *testing.T, scheme string) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	...
	_, chain, _ = GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func testLogReorgs(t *testing.T, scheme string) {
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	defer blockchain.Stop()

	rmLogsCh := make(chan RemovedLogsEvent)
	blockchain.SubscribeRemovedLogsEvent(rmLogsCh)
	_, chain, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
...
func testSideLogRebirth(t *testing.T, scheme string) {
	...
	genDb, sideChain, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, sideChain[len(sideChain)-1], qmpow.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
...
func testReorgSideEvent(t *testing.T, scheme string) {
	...
	_, replacementBlocks, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 4, func(i int, gen *BlockGen) {
...
func testCanonicalBlockRetrieval(t *testing.T, scheme string) {
	_, gspec, blockchain, err := newCanonical(qmpow.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	_, chain, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 10, func(i int, gen *BlockGen) {})
...
func testEIP155Transition(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 4, func(i int, block *BlockGen) {
		var (
	...
	blocks, _ = GenerateChain(config, blocks[len(blocks)-1], qmpow.NewFaker(), genDb, 4, func(i int, block *BlockGen) {
		var (
			tx      *types.Transaction
			err     error
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
...
func testEIP161AccountRemoval(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
func testInsertReceiptChainRollback(t *testing.T, scheme string) {
	...
	ancientChain, _ := NewBlockChain(ancientDb, DefaultCacheConfigWithScheme(scheme), gspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
