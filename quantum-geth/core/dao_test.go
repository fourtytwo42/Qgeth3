...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	genDb, prefix, _ := GenerateChainWithGenesis(gspec, qmpow.NewFaker(), int(forkBlock.Int64()-1), func(i int, gen *BlockGen) {})
	...
	proBc, _ := NewBlockChain(proDb, nil, progspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	defer proBc.Stop()
	...
	conBc, _ := NewBlockChain(conDb, nil, congspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
	defer conBc.Stop()
	...
	for i := int64(0); i < vars.DAOForkExtraRange.Int64(); i++ {
		...
		bc, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, congspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
		...
		// blocks, _ = GenerateChain(&proConf, conBc.GetBlockByHash(conBc.CurrentBlock().Hash()), qmpow.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
		// if _, err := conBc.InsertChain(blocks); err == nil {
		// 	t.Fatalf("contra-fork chain accepted pro-fork block: %v", blocks[0])
		// }

		// Create a proper no-fork block for the contra-forker
		blocks, _ = GenerateChain(&conConf, conBc.GetBlockByHash(conBc.CurrentBlock().Hash()), qmpow.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
		if _, err := conBc.InsertChain(blocks); err != nil {
			t.Fatalf("contra-fork chain didn't accepted no-fork block: %v", err)
		}
		// Create a no-fork block, and try to feed into the pro-fork chain
		bc, _ = NewBlockChain(rawdb.NewMemoryDatabase(), nil, progspec, nil, qmpow.NewFaker(), vm.Config{}, nil, nil)
		...
		bc.Stop()
		blocks, _ = GenerateChain(&conConf, proBc.GetBlockByHash(proBc.CurrentBlock().Hash()), qmpow.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
		if _, err := proBc.InsertChain(blocks); err == nil {
			t.Fatalf("pro-fork chain accepted contra-fork block: %v", blocks[0])
		}
		// Create a proper pro-fork block for the pro-forker
		blocks, _ = GenerateChain(&proConf, proBc.GetBlockByHash(proBc.CurrentBlock().Hash()), qmpow.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
		if _, err := proBc.InsertChain(blocks); err != nil {
